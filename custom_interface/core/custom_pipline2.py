...
def get_text_and_region_one_v2(pipline: CustomPipline, frame: np.array) -> tuple[str, str] | None:
    ...
    confidences, texts) = unzip(pipline([frame]))
    if texts[0] and region_names:
        if region_names[0][0] == 'eu_ua_ordlo_dpr':
...
if __name__ == '__main__':
    ...
    frame = cv2.imread('/Users/adilet/nomeroff-net/2b68f3fb-182d-4ca8-b1d4-6152bc1cdea6.jpg')
    res = get_text_and_region_one(pipline, frame)
    # cv2.imwrite('/Users/adilet/nomeroff-net/media/2_q.jpg', res[2])
    print(res[0], res[1])